...
from collectors.youtube_scanner import rematch_unmatched_videos, run_youtube_scan
...
from collectors.demo_reviews import run_demo_review_snapshots

logger = logging.getLogger(__name__)


async def run_steam_extras() -> None:
    """Run update tracking → achievement stats → demo reviews sequentially.

    All three use steam_limiter. Grouped into a single job to avoid
    concurrent SQLite write contention.
    """
    logger.info("Starting steam_extras pipeline: updates → achievements → demo reviews")
    await run_update_tracking()
    await run_achievement_stats()
    await run_demo_review_snapshots()
...
__all__ = [
    ...
    "run_reddit_scan",
    "run_dev_profiles",
    "run_demo_review_snapshots",
    "run_steam_extras",
    "rematch_unmatched_videos",
]
