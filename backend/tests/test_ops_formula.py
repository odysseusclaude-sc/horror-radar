...
def _extract_component_keys_from_ops() -> set[str]:
    ...
    `_calculate_ops_for_game()`'s `components = {...}` dict.
    """
    source = OPS_PY.read_text()
    tree = ast.parse(source)

    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "_calculate_ops_for_game":
    ...
    raise AssertionError(
        ...
        "_calculate_ops_for_game() — ops.py structure changed."
    )
